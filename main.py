...
if Enter_energy==1:
    ...
    pathData='Data8'
    name = '8MeV'
    mean_energy_dep=15078.96   # LET keV/4micro m
elif Enter_energy==2:
    pathFiles='Data1_8/Res/FilesOfInterest.txt'
    pathData='Data1_8'
    name='1_8MeV'
    mean_energy_dep=57960.49666363634  # LET keV/4micro m
elif Enter_energy==3: #1.46
    pathFiles='Data1_4/Res/FilesOfInterest.txt'
    pathData='Data1_4'
    name='1_4MeV'
    mean_energy_dep=69045.56688785045  # LET keV/4micro m
elif Enter_energy==4:
    pathFiles='Data1_2/Res/FilesOfInterest.txt'
    pathData='Data1_2'
    name='1_2MeV'
    mean_energy_dep=75369.00045882349   # LET keV/4micro m

print('\n\n-----------------------------------------------------------------------')
d = float(input('Enter dose in Gy:'))  # user inputs the dose in Gy
print('-----------------------------------------------------------------------')
numberProt = dose(d,mean_energy_dep)  # Calculate fluence
#numberProt = 2*1e6 # used for testing

print('\n\n-----------------------------------------------------------------------')
NumberCell = int(input('Enter cell count:'))  # input cell ammount
print('-----------------------------------------------------------------------')
x_cell,y_cell,x_nucleus,y_nucleus=celldist(NumberCell)  # cell distrebution func
...
for i in range(iterations):  # loop to iradiate cells
    start = timer()
    x,y = MC(max_prot_per)  # montecarlo for protons
    """
    Interaction takes a cell/nucleus hit matrix and returns the same matrix with
    the hits added to the matrix
    """
...
# chooseProton assign protons from the proton-pool to each cell hit
proton_randoom_draw_matrix,proton_randoom_draw_matrix_nucleus = chooseProton(hitsCell,hitsNucleus,pathFiles,max_hit)


# Ting tar tid opens the proton data and reads it into the cell/nuc
